...
exports.module = function (connection, stanza) {
	//console.log("["+listenerTypes[connection.listenerType]+"]["+connection.listenerId+"][" + connection.ip +":"+ connection.port+"]["+connection.jid+"][Info]:Warface");
					...
					switch(stanza.xmlns){		
						case "jabber:client":
							if(global.config.pingEnable == true && connection.pingTime != -1 && stanza.attrs.id != null && stanza.attrs.id == "ping"+connection.pingId){
								...
								//console.log("["+listenerTypes[connection.listenerType]+"]["+connection.listenerId+"][" + connection.ip +":"+ connection.port+"]["+connection.jid+"][Info]:Ping "+pingResult+" ms");
								...
								if(global.config.pingAnonceToMasterservers == true && connection.listenerType == 0 && connection.username != "masterserver" && connection.username != "dedicated"){
									for(keyConnection in global.connectionsOnline){
