...
exports.module = function (stanza) {
    ...
    global.db.warface.profiles.findOneAndUpdate({ username: profileObject.username }, { $set: { status: new_status } }, { projection: { "_id": 1 } }, function (errUpdate, resultUpdate) {
        ...
        if(new_status == 21 || new_status == 85 || new_status == 149) {

            //new_status == 37 если человек в катке, запускать таймер, если по окончанию таймера статусов больше не приходило, кикать

            gameroom_leave.module({ attrs: { from: playerObject.online_id } }, false, true, 2);
            //global.xmppClient.request(playerObject.online_id, new ltxElement("gameroom_on_kicked", {"reason":"2"}));
            scriptProfile.giveNotifications(profileObject.username, [{ type: 8, params: { data: "@messagebox_you_are_kicked_timeout" } }], function (nAddResult) {
            });
